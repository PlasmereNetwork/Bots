...
public class OplistModificationCommand extends Command {

    private final ScreenWriter writer;

    private OplistModificationCommand(List<String> channelIDs, ScreenWriter writer) {
        super(channelIDs, "\\.(op|deop) .+");
        this.writer = writer;
    }
    ...
    public static class Factory implements ListenerFactory {
        ...
        public Listener generateListener(Bot bot, ChannelFactory factory) {
            return new OplistModificationCommand(channelIDs, writer);
        }
    }
}
